variable "folder_id" {
  description = "Bare numeric ID of the folder the IAM resources apply to (e.g. \"123456789012\")."
  type        = string

  validation {
    condition     = can(regex("^[0-9]+$", var.folder_id))
    error_message = "folder_id must be the bare numeric folder ID (e.g. \"123456789012\")."
  }
}
...
variable "mode" {
  ...
  validation {
    ...
    error_message = "google_folder_iam_policy cannot be combined with google_folder_iam_audit_config (they fight over the policy). Disable audit_config_enabled or empty audit_configs when mode = \"policy\"."
  }
}

variable "members" {
  description = "Map of IAM grants keyed by an arbitrary identifier. Each entry grants all roles to one member, creating one google_folder_iam_member per role; a condition applies to all of the entry's roles. Only used when mode = \"member\"."
  type = map(object({
    member = string
    roles  = list(string)
  ...
  validation {
    condition = alltrue(flatten([
      for m in var.members : [
        for role in m.roles :
        can(regex("^roles/.+", role)) || can(regex("^organizations/[0-9]+/roles/.+$", role))
      ]
    ]))
...
variable "bindings" {
  description = "Map of IAM bindings keyed by an arbitrary identifier. Each entry creates one google_folder_iam_binding. Only used when mode = \"binding\". Roles must be unique across bindings."
  type = map(object({
    role    = string
    members = list(string)
    condition = optional(object({
  ...
  validation {
    condition = alltrue([
      for b in var.bindings :
      can(regex("^roles/.+", b.role)) || can(regex("^organizations/[0-9]+/roles/.+$", b.role))
    ])
  ...
  validation {
    ...
    error_message = "bindings must not grant the same role twice; google_folder_iam_binding is authoritative per role."
  }
...
variable "policy_data" {
  description = "JSON-encoded IAM policy (build it with a google_iam_policy data source or jsonencode). Creates one google_folder_iam_policy when mode = \"policy\"."
  type        = string
  default     = null
...
variable "audit_configs" {
  description = "Map of audit log configurations keyed by an arbitrary identifier (typically the service name). Each entry creates one google_folder_iam_audit_config. Services must be unique across entries."
  type = map(object({
    service = string
  ...
  validation {
    ...
    error_message = "audit_configs must not configure the same service twice; google_folder_iam_audit_config is authoritative per service."
  }
...
variable "audit_config_enabled" {
  description = "Master toggle for google_folder_iam_audit_config resources. Enabled by default; set false to manage no audit configs."
  type        = bool
  default     = true

# stackit/opensearch_credential

Map-keyed module for STACKIT OpenSearch credentials on an existing
OpenSearch instance (see stackit/opensearch_instance).
...
| `instance_id` | `string` | — | ID of the OpenSearch instance the credential is created on (e.g. from stackit/opensearch_instance's `instances` output, validated). Changing it replaces the credential. |
...
There are no `username`, `roles` or `index` inputs on this resource —
username and password are both API-generated. A `stackit_opensearch_user`
resource does not exist; this credential resource is the provider's user
resource.
...
Unlike the Redis and Valkey credential resources there is no
`load_balanced_host` output — that attribute does not exist on
OpenSearch credentials.

## Example

```hcl
module "opensearch_credential" {
  source = "git::ssh://git@github.com/example/terraform-modules.git//modules/stackit/opensearch_credential?ref=v2.8.0"

  credentials = {
    "app" = {
      project_id  = "12345678-1234-1234-1234-123456789012"
      instance_id = module.opensearch_instance.instances["app-search"].instance_id
      region      = "eu01"
      rotate_when_changed = {
        rotation = "2026-09-12"
      }
...
`stackit_opensearch_credential` ← `{project_id},{region},{instance_id},{credential_id}`

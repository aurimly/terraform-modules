# stackit/opensearch_instance

Map-keyed module for STACKIT OpenSearch instances.
...
| `version` | `string` | — | OpenSearch version (e.g. `2.x`, free string — confirm the versions the chosen plan supports against the STACKIT docs). Updates in place. |
| `plan_name` | `string` | — | Plan name (e.g. `stackit-opensearch-1.5.0-xsmall-replica`); list available plans consumer-side via the `stackit_opensearch_plans` data source (validated non-empty). Updates in place. |
...
| `graphite` | `string` | `null` | Graphite endpoint (`host:port`); enables Graphite monitoring. Updates in place. |
| `java_garbage_collector` | `string` | `null` | Garbage collector to use for OpenSearch. Updates in place. |
| `java_heapspace` | `number` | `null` | Heap memory (MB) allocated by the JVM for OpenSearch. Updates in place. |
| `java_maxmetaspace` | `number` | `null` | Memory (MB) used by the JVM to store metadata for OpenSearch. Updates in place. |
...
| `plugins` | `list(string)` | `null` | Plugins to install (must be supported plugin names). `repository-s3` and `repository-azure` are always enabled and cannot be disabled. Updates in place. |
...
| `tls_protocols` | `list(string)` | `null` | TLS protocol versions to use. Updates in place. |
...
no connection info; credentials come from the
`stackit_opensearch_credential` resource.

## Example

```hcl
module "opensearch_instance" {
  source = "git::ssh://git@github.com/example/terraform-modules.git//modules/stackit/opensearch_instance?ref=v2.1.0"

  instances = {
    "app-search" = {
      project_id = "12345678-1234-1234-1234-123456789012"
      name       = "example-app-search"
      version    = "2.x"
...
- **Renaming a map key destroys and recreates the instance — data-loss
  risk on a search cluster.** The same applies to changing `name`,
...
- The `repository-s3` and `repository-azure` plugins are always enabled
  upstream and cannot be disabled; only other plugins can be added via
  `parameters.plugins`.
- Plan-time validations mirror the provider's plan-time validators
  ...
  `monitoring_instance_id`) as forward-checking; plugin names are left
  unvalidated — the provider rejects unsupported plugin names at apply.
...
`stackit_opensearch_instance` ← `{project_id},{region},{instance_id}`

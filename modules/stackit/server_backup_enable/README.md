# stackit/server_backup_enable

Map-keyed module enabling the STACKIT server backup service for existing
servers. Pair with stackit/server_backup_schedule for the backup
schedules themselves; use this module to enable the service explicitly —
starting 26.09.2026 schedule resources no longer enable it implicitly.

## Inputs

| Name | Type | Default | Description |
|---|---|---|---|
| `server_backup_enables` | `map(object)` | — | Map of backup enablements keyed by an arbitrary unique ID. |

### `server_backup_enables` object

| Attribute | Type | Default | Description |
|---|---|---|---|
| `project_id` | `string` | — | STACKIT project UUID (validated). Changing it replaces the enable. |
| `server_id` | `string` | — | UUID of the server to enable backups on (validated). Changing it replaces the enable. |
| `backup_policy_id` | `string` | `null` | UUID of the backup policy to apply (validated). Changing it fails at apply — the provider does not support updating this resource; recreate the entry (or rollover the map key) instead. |
...
`server_backup_enables` — map of enable key => object:
...
- **Use only one enable resource per server** — a duplicate enable is
  ...
  state.
- **destroy actually disables the backup service** — the provider calls
  ...
  its key) destroys and recreates: backups turn off and back on with a
  brief gap in between.
- **The implicit enable in schedule resources is deprecated and will be
  removed 26.09.2026.** Run this module alongside
  stackit/server_backup_schedule (see its README example for the
...
- Plan-time validations mirror the provider's plan-time validators
  (UUIDs for `project_id`/`server_id`/`backup_policy_id`). The
  `backup_policy_id` update failure is not caught plan-time — the
  provider declares in-place updates for `backup_policy_id` but its

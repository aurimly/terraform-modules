variable "role_assignments" {
  description = "Map of STACKIT role assignments on the folder keyed by an arbitrary identifier. Each entry assigns one role to one subject."
  type = map(object({
    resource_id = string
    role        = string
    subject     = string
  ...
  validation {
    ...
    error_message = "resource_id must be set to the folder ID (folder UUID)."
  }

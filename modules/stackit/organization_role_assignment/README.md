# stackit/organization_role_assignment

Map-keyed module for STACKIT role assignments on an organization.
...
|---|---|---|---|
| `resource_id` | `string` | — | The organization ID (UUID) the role is assigned to (the provider UUID-validates this attribute). |
...
```hcl
# resource_id may differ per entry — each entry targets its own organization.
role_assignments = {
  "org-owner" = {
...
- Chaining: pass the organization ID as `resource_id` (Terragrunt
  `dependency` on the unit that manages the organization).
- The runner's service account needs rights to manage role assignments on
  the target resource (owner-level role on the organization covers this —
...
`stackit_authorization_organization_role_assignment` ← `{resource_id},{role},{subject}`

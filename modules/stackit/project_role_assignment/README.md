# stackit/project_role_assignment

Map-keyed module for STACKIT role assignments on a project.
...
|---|---|---|---|
| `resource_id` | `string` | — | The project UUID the role is assigned to (the provider UUID-validates this attribute) — `stackit/project`'s `project_id` output. |
...
```hcl
# resource_id may differ per entry — each entry targets its own project.
# e.g. chain stackit/project's project_id output via a Terragrunt dependency.
role_assignments = {
  "project-owner" = {
...
- Chaining: pass `stackit/project`'s `project_id` output as `resource_id`
...
`stackit_authorization_project_role_assignment` ← `{resource_id},{role},{subject}`
